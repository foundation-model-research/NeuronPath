...
from timm_mlp_edit import Mlp as _Mlp
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, path = None, edit_operation = 'remove') -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        x_, ffn = self.drop_path2(self.ls2(self.mlp(self.norm2(x), path = path, edit_operation = edit_operation)))
        x = x + x_
        return x, ffn
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, path = None, edit_operation = 'remove') -> torch.Tensor:
        ...
        x, ffn = self.forward_features(x, path = path, edit_operation = edit_operation,)
        
        logits = self.forward_head(x)

        prob = F.softmax(logits, dim=1)

        return logits, ffn, prob
