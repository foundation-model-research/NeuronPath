...
import modeling_mae_edit as modeling_mae
    ...
    for batch_idx, batch in enumerate(epoch_iterator):
        ...
        with open(os.path.join(methods_dict[args.method], f"imgnet_all-{int(l)}.rlt.jsonl"), 'r') as f:
            ...
            if args.method == "influence_pattern":
                for _sub in _data:
                    for _sub_sub in _sub["path"]:
                        data.append({"ja":[_sub_sub]})
            else:
                for _sub in _data:
                    for _sub_sub in _sub:
                        data.append(_sub_sub)
            ...
            for one_data in sub_data:
                if args.method=="base":
                    path.append([int(n) for n in one_data['base'][0]])
                else:
                    path.append([int(n) for n in one_data['ja'][0]])
            path = torch.tensor(path).to(device)

        logits, _, prob = model(x, path = path, edit_operation = "remove")
        remove_logits = F.softmax(logits, dim=1)[:, l]
        remove_label = torch.argmax(F.softmax(logits, dim=1), dim = 1)

        logits, _, prob = model(x, path = path, edit_operation = "enhance")
        enhance_logits = F.softmax(logits, dim=1)[:, l]
        enhance_label = torch.argmax(F.softmax(logits, dim=1), dim = 1)

        logits, _, prob = model(x)
        base_logits = F.softmax(logits, dim=1)[:, l]
        base_label = torch.argmax(F.softmax(logits, dim=1), dim = 1)

        with jsonlines.open(os.path.join(args.output_dir, args.output_prefix + '-' + str(int(l)) + '.rlt' + '.jsonl'), 'a') as fw:
            for _remove, _enhance, _base, p, _base_label, _remove_label, _enhance_label in zip(remove_logits, 
                                                   enhance_logits,
                                                   base_logits, 
                                                   path,
                                                   base_label,
                ...
                res_dict["base_label"].append(float(_base_label))
                res_dict["remove_label"].append(float(_remove_label))
                res_dict["enhance_label"].append(float(_enhance_label))
