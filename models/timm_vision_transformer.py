...
from timm_mlp import Mlp as _Mlp
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, tmp_score=None, imp_pos=None, imp_op=None) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        x_, ffn = self.drop_path2(self.ls2(self.mlp(self.norm2(x), tmp_score=tmp_score, imp_pos=imp_pos, imp_op=imp_op)))
        x = x + x_
        return x, ffn
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, tgt_layer=-1, tmp_score=None, tgt_label=None, imp_pos=None, imp_op=None, path=None, path_weights=None,) -> torch.Tensor:
        if path is not None:
            x, ffn = self.forward_features(x, tgt_layer=tgt_layer, tmp_score=tmp_score, path=path, path_weights=path_weights)
        else:
            x, ffn = self.forward_features(x, tgt_layer=tgt_layer, tmp_score=tmp_score, )
        
        logits = self.forward_head(x)

        if (path is not None) and ((path == -1).any()):
            gradient = []
